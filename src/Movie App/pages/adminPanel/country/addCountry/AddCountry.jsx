...
import { useAddCountryMutation } from "../../../../redux/services/movieDatabase";
...
const AddCountry = ({ history }) => {
  ...
  const SubmitHandler = () => {
    ...
    addNewCountry(formData)
      .unwrap()
      .then((r) => {
        toast.success(`${Formik.values.title} add to Countrues `, {
          autoClose: 1100,
          position: "top-right",
        });
        setLoadingButton(false);

        setTimeout(() => history.push("countrieslist"), 800);
      })
      .catch((error) => {
        toast.error(`error`, {
          autoClose: 1100,
  ...
  return (
    <div className=" my-10 h-screen  mx-6 sm:mx-10 md:mx-28">
      <div className="text-[23px] font-bold mt-10 mb-6 ">Add New Country </div>
